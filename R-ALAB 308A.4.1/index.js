...
const API_KEY = "live_QV4DKDTpL3OkYYTi6qRS10feE5O1WSBwBUUBXsfyeIgNbUCaqk0nVtBTENHzsK0M";
...
breedSelect.addEventListener("change", async function (event) {
  ...
  try {
    const response = await fetch(
      `https://api.thecatapi.com/v1/images/search?:${selectedBreedId}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
